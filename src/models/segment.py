...
def get_segment_model(inage_size):
    ...
    model.add(L.Conv2D(filters=30, kernel_size=(3, 3)))
    model.add(L.BatchNormalization())
    model.add(L.Activation('relu'))
    model.add(L.Conv2D(filters=50, kernel_size=(3, 3)))
    model.add(L.MaxPooling2D(pool_size=(2, 2)))
    model.add(L.Activation('relu'))
    model.add(L.Dropout(0.15))

    # model.add(L.Conv2D(filters=100, kernel_size=(4, 4)))
    # model.add(L.MaxPooling2D(pool_size=(2, 2)))
    # model.add(L.Activation('relu'))
    # model.add(L.Dropout(0.15))

    model.add(L.Conv2D(filters=150, kernel_size=(5, 5)))
    model.add(L.BatchNormalization())
    model.add(L.Activation('relu'))
    model.add(L.MaxPooling2D(pool_size=(2, 2)))
    model.add(L.Activation('relu'))
    ...
    model.add(L.Flatten())
    model.add(L.Dense(800, activation='relu'))
